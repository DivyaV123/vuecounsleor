...
const itemsPaginated = computed(() => {
  if (counselor.weekendEnquiries && counselor.weekendEnquiries.items) {
    return counselor.weekendEnquiries.items
  }
});
...
const confirmRefresh = async () => {
  ...
  await counselor.getWeekendEnquiries(currentPage.value - 1);
...
const searchParams = async (search) => {
  ...
  if (search.length > 0) {
    ...
    await counselor.getWeekendEnquiries(0, search);
  } else {
    searchQuery.value = "";
...
const commentmodal =(id,name)=>{
  ...
  propValues.commentdata="weekend enq"
}
...
const pageChange = async (page) => {
  ...
  if (searchQuery.value.length > 0) {
    await counselor.getWeekendEnquiries(page, searchQuery.value);
  } else {
    await counselor.getWeekendEnquiries(page);
  }
  pageLoading.value = false;
  window.scroll({
    top: 0,
...
<template>
  ...
  <div class="flex justify-start w-full bg-[#151D2B] sticky top-[71px] z-[9] pb-[6px] pt-2">
  <jb-button
    label="search"
    ...
    <tbody>
      ...
      >
        ...
        <td data-label="Timings">
          <small>
            <span v-if="client.enquiry?.class_timing" class="text-amber-100"> 
        ...
        <td data-label="Comments">      
         ...
         <td data-label="DOE">
          <small class="text-teal-400 font-semibold" >
            {{ dayjs(client.created ).format("DD-MMM-YY hh:mm A") }}
          </small>
           <!-- <small class="text-red-400 font-semibold" v-else>
            {{ "No enquiry added" }}
          </small> -->
        </td>
