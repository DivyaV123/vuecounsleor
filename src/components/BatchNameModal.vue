...
const recs = computed(() => props.batchname);
const currentPage = ref(1);

const hasError = ref(false)
const perPage = 50;
const itemsCount =computed(() =>followup.students_total);
const itemsPaginated = computed(() =>followup.students);
...
const pageChange = async (page) => {
    ...
    // } else {
        await followup.getstudents(page,recs.value);
    // }
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getstudents(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getstudents(currentPage.value,recs.value );
    }
...
onBeforeMount(async (page) => {
   pageLoading.value = true
 await followup.getstudents(currentPage.value-1,recs.value) 
  pageLoading.value = false
})
</script>
...
<div v-if="pageLoading" wire:loading
  ...
  >
    <div>
      ...
      <div class="space-y-3 ">
        <div v-if="itemsCount>0"> <table >
    <thead class="sticky top-[48px] z-[9] bg-gray-900">
      <tr>
      <th>Name</th>
        <th>Academic</th>
        <th>Mobile No.</th>
  ...
  <td data-label="Academics">
        ...
        <td  data-label="Mobile"  >   
          <span
         v-for="(client1, index1) in client.contact"
        :key="index1%2===1" >
...
<!--          
  ...
  <td  data-label="Call Count" v-if="client.records&&client.records.length  ">         
    ...
    <td v-else><jb-button :icon="mdiMicrophone" small /> </td>
     ...
     <td data-label="Status" v-if="client.records&&client.records.length  ">
        ...
        <td v-else> <span  class="text-gray-400 dark:text-gray-400 font-semibold">No status</span></td>
