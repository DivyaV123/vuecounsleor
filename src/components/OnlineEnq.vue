...
const itemsPaginated = computed(() => {
  if(counselor.onlineEnqs && counselor.onlineEnqs.items){
    return counselor.onlineEnqs.items
  }
  // const getItems = [];
...
const confirmRefresh = async () => {
  ...
  await counselor.getOnlineEnq(currentPage.value-1);
...
const searchParams = async (search) => {
  if (search.length > 0) {
    ...
    counselor.onlineEnqs(1, search);
  } else {
    searchQuery.value = "";
    counselor.onlineEnqs();
  }
};
...
const pageChange = async (page) => {
  ...
  //   await counselor.getOnlineEnq(page, searchQuery.value);
  // } else {
    await counselor.getOnlineEnq(page);
  // }
  pageLoading.value = false
  window.scroll({
    top: 0,
    ...
    <tbody>
      <tr v-for="(client, index) in itemsPaginated" :key="client.id"
        ...
        <td data-label="Course">
          ...
          <span v-for="(course, i) in client.online_courses" :key="i">
            <small class="text-gray-100 dark:text-gray-200">
              {{ course.course.name }}
              <span v-if="course.branch" class="text-red-50"> - {{ course.branch.name }}</span>
              <span v-if="course.mode_class" class=" text-lime-200"> - {{ course.mode_class.name }}</span>
