...
const confirm = async () => {
  ...
  for (const key in formValues) {
    if (key === 'comment' && (formValues[key].length === 0)) {
      valid = false;
  ...
  if (valid) {
    ...
    let response = await counselor.addCorporateComment(formSubmit)
  ...
  <overlay v-show="value" @overlay-click="cancel">
    <card-component v-show="value" :title="title" class="shadow-lg w-full max-h-modal md:w-3/5 z-50 overflow-y-scroll"
      ...
      <div class="pl-4 space-y-3">
        <card-component>
          <field>
            <control v-model="formState.comment" help="Comment" placeholder="Comment" :error="
