...
const itemsPaginated = computed(() => {
    if(counselor.callBack && counselor.callBack.items){
        return counselor.callBack.items
    } 
}
);

const itemsCount = computed(() => {
    if(counselor.callBack && counselor.callBack.total){
        return counselor.callBack.total
    } 
}
);
...
const confirmRefresh = async () => {
  ...
  if(s){
  await counselor.getCallBack(currentPage.value-1,s);
    }
    else{
      await counselor.getCallBack(currentPage.value-1);
    }
...
const pageChange = async (page) => {
  ...
  await counselor.getCallBack(page, s);
  pageLoading.value = false
  window.scroll({
    top: 0,
    left: 0,
...
<template>
  ...
  <div class="sticky top-[76px] z-[1] bg-[#151D2B]">
  ...
  <table class="">
    <thead class="sticky top-[170px] z-[1] bg-gray-900">
      <tr>
        <th v-if="checkable" />
        <th >Contact</th>
