...
const recs = computed(() => props.callback);
const currentPage = ref(0);

const hasError = ref(false)
const perPage = 50;
const itemsPaginated  = computed(() => followup.callback)
const itemsCount =computed(() => followup.callback_total)
...
const pageChange = async (page) => {
   ...
   await followup.getcallback(currentPage.value-1,recs.value)
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getcallback(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getcallback(currentPage.value,recs.value );
    }
...
<!-- 
        <td  data-label="Branch" v-if="client.courses===null">
         <small v-for="(client3, index1) in  client.other_courses"
    ...
    <td v-else>   <small  class="text-gray-400 dark:text-gray-400 font-semibold">No status</small></td>
