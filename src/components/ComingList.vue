...
const recs = computed(() => props.coming);

const currentPage = ref(1);

const hasError = ref(false)
const perPage =50;
const itemsPaginated = computed(() => followup.coming);
const itemsCount =computed(() => followup.coming_total);
...
const pageChange = async (page) => {
    ...
    // } else {
        await followup.getcoming(page,recs.value);
    // }
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getcoming(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getcoming(currentPage.value,recs.value );
    }
...
const coming=computed(()=>{
   return followup.coming  
})

onBeforeMount(async (page) => {
  pageLoading.value = true
  await followup.getcoming(currentPage.value-1,recs.value) 
  pageLoading.value = false 
})
</script>
