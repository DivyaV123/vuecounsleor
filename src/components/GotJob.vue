...
import ModalGotJob from "@/components/ModalGotJob.vue";
...
const itemsPaginated = computed(() => {
  if(counselor.gotJob && counselor.gotJob.items){
      return counselor.gotJob.items
  } 
}
);
const itemsCount = computed(() => {
  if(counselor.gotJob && counselor.gotJob.total){
      return counselor.gotJob.total
  } 
}
);
...
const isModalGJActive = ref(false);

const openGJModal = (id, name) => {
  isModalGJActive.value = true;
  propValues.id = id;
...
const pageChange = async (page) => {
  ...
  await counselor.getGotJob(page);
  pageLoading.value = false
  window.scroll({
    top: 0,
    left: 0,
...
<template>
  ...
  <modal-got-job v-if="isModalGJActive" v-model="isModalGJActive" :title="'Add got job of ' + propValues.name"
    ...
    <tbody>
      <tr v-for="(client, index) in itemsPaginated" :key="client.id"
        ...
        <!-- <td data-label="Actions">
          <small class="text-gray-500 dark:text-gray-300">
            <jb-button label="Got job" color="orange" @click="openGJModal(client.CallRecordID, client.CallRecord.name)" small
              class="mr-1 mb-1" />
          </small>
        </td> -->
      </tr>
