...
import ModalCorporateComment from "@/components/ModalCorporateComment.vue";
...
const itemsPaginated = computed(() => {
  if (counselor.allEnquiries && counselor.allEnquiries.items) {
    return counselor.allEnquiries.items
  }
});
...
const confirmRefresh = async () => {
  ...
  await counselor.getAllEnquiries(currentPage.value - 1, searchQuery.value)
...
const searchParams = async (search) => {
  ...
  if (search.length > 0) {
    ...
    await counselor.getAllEnquiries(0, search);
  } else {
    searchQuery.value = "";
...
const openOnlineModal = (id, name) => {
  ...
  propValues.commentdata="counselor"
};
...
const pageChange = async (page) => {
  ...
  if (searchQuery.value.length > 0) {
    await counselor.getAllEnquiries(page, searchQuery.value);
  } else {
    await counselor.getAllEnquiries(page);
  }
  pageLoading.value = false;
  window.scroll({
    top: 0,
...
<template>
    ...
    :title="'Add comment for ' + propValues.name"
  ...
  <div class="w-full bg-[#151D2B] sticky top-[71px] z-[9] pb-2 pt-2">
  <jb-button
    label="search"
  ...
  <table class="mt-6">
    ...
    <tbody>
      ...
      >
        ...
        <td data-label="DOE">
          <small> {{ dayjs(client.created).format("DD-MMM-YY hh:mm a") }} </small>  
        </td>
        <td data-label="Branch">
          <small   v-for="(item, index) in client.courses"
        :key="item.id">{{ item.branch.name }}
        <br/>
        &nbsp;
      </small>
          <!-- <small v-if="client.courses[0] && client.courses[0].branch"> {{ client.courses[0].branch.name }} </small>   -->
        </td>
        <td data-label="Academics">
          ...
          <small v-if="client.education.yop && client.enquiry.experienced === false" class="text-gray-400">
        ...
        <td data-label="Status">
          <small v-if="client.records?.length > 0" class="text-gray-400 dark:text-gray-300 font-semibold capitalize">
        ...
        <!-- <td v-else data-label="Status"> 
          <small  class="text-gray-400 dark:text-gray-400 font-semibold">No status</small>
        </td> -->
          ...
          <small class="text-gray-500 dark:text-gray-300 capitalize text-sm">
            <span >
              <jb-button
                ...
                small
                outline
                @click="openOnlineModal(client.id, client.name)"
