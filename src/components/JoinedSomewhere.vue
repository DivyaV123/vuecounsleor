...
<template>
  <modal-joined-somewhere v-if="isModalJSActive" v-model="isModalJSActive"
    :title="'Add joined somewhere for ' + propValues.name" :id="propValues.id" @confirm="confirmRefresh" has-cancel>
  </modal-joined-somewhere>
    ...
    <tbody>
      <tr v-for="(client, index) in itemsPaginated" :key="client.id"
        ...
        <!-- <td data-label="Actions">
          <small class="text-gray-500 dark:text-gray-300">
            <jb-button label="Joined somewhere" color="danger" @click="openJSModal(client.CallRecordID, client.CallRecord.name)" small
              class="mr-1 mb-1" />
          </small>
        </td> -->
      </tr>
...
import ModalJoinedSomewhere from "@/components/ModalJoinedSomewhere.vue";
...
const itemsPaginated = computed(() => {
  if(counselor.joinedSomewhere && counselor.joinedSomewhere.items){
      return counselor.joinedSomewhere.items
  } 
}
);
const itemsCount = computed(() => {
  if(counselor.joinedSomewhere && counselor.joinedSomewhere.total){
      return counselor.joinedSomewhere.total
  } 
}
);
...
const confirmRefresh = async () => {
  ...
  await counselor.getJoinedSomewhere();
...
const isModalJSActive = ref(false);

const openJSModal = (id, name) => {
  isModalJSActive.value = true;
  propValues.id = id;
...
const pageChange = async (page) => {
  ...
  await counselor.getJoinedSomewhere(page);
  pageLoading.value = false
  window.scroll({
    top: 0,
    left: 0,
