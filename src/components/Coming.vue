...
const itemsPaginated = computed(() => counselor.coming);
const itemsCount = computed(() => counselor.comingCount);

const confirmRefresh = async () => {
  pageLoading.value = true
  await counselor.getComing();
...
const pageChange = async (page) => {
  ...
  await counselor.getComing(page);
  pageLoading.value = false
  window.scroll({
    top: 0,
    left: 0,
    ...
    <tbody>
      <tr v-for="(client, index) in itemsPaginated" :key="client.enquiry.id"
        ...
        <td data-label="Actions">
          <small class="text-gray-500 dark:text-gray-300">
            <jb-button label="Follow up" color="orange" small class="mr-1 mb-1" />
            <jb-button @click="openStatusModal(client)" label="Change status" color="info" small class="mr-1 mb-1" />
          </small>
        </td>
      </tr>
    </tbody>
