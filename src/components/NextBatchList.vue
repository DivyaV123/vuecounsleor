...
const email =ref("")
const number =ref("")
const itemsPaginated  = computed(() => followup.nextbatch)
const itemsCount =computed(() => followup.nextbatch_total)

const maskNumber = (number) => {
  return number.replace(/\d(?=\d{4})/g, "*")
}
const recs = computed(() => props.nextbatch)
...
const pageChange = async (page) => {
    ...
    // } else {
        await followup.getnextbatch(page,recs.value);
    // }
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getnextbatch(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getnextbatch(currentPage.value,recs.value );
    }


const nextbatch=computed(()=>{
   return followup.nextbatch  
})
...
onBeforeMount(async () => {
    pageLoading.value = true
  await followup.getnextbatch(currentPage.value-1,recs.value)
   pageLoading.value = false
