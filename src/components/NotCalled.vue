...
const itemsPaginated  = computed(() => followup.notcalled)
const itemsCount =computed(() => followup.notcalled_total)

const maskNumber = (number) => {  
  return number.replace(/\d(?=\d{4})/g, "*")
}

const recs = computed(() => props.notcalled);
...
const pageChange = async (page) => {
    ...
    currentPage.value = page+1;
        await followup.getnotcalled(page,recs.value);
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getnotcalled(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getnotcalled(currentPage.value,recs.value );
    }
...
const notcalled=computed(()=>{
   return followup.notcalled  
})

const isModalActive = ref(false)
onBeforeMount(async () => {
  pageLoading.value = true
  await followup.getnotcalled(currentPage.value-1,recs.value)
  pageLoading.value = false
...
<template>
 <div v-if="pageLoading" wire:loading
  ...
  >
    ...
    <card-component
      class="shadow-lg w-full h-screen md:w-5/5 z-50 overflow-y-scroll mt-[46px]"
       title="Not Called data" 
...
<!-- {{client.education==null}} --> 
       ...
       <td  data-label="Mobile" v-if="client && client.contact" >   
      <span v-for="(client1, index1) in client.contact" :key="index1%2===1" >
        ...
        <td  data-label="Email" v-if="client && client.contact">
         <span v-for="(client2, index1) in client.email" :key="index1%2===1">
        <small
            class="text-gray-500 dark:text-gray-300">
           <a class=""
...
<!-- {{client.other_courses}} -->
       <td  data-label="Courses" v-if=" client && client.courses&&client.other_courses">
        ...
        <small v-if="client3.course">
          <span >{{client3.course.name}}</span> -- <span class="text-emerald-400 text-md"> {{client3.branch.name }}</span>
          <br/>
          </small>
             &nbsp;
         </small>    
        </td>
          <td  data-label="Courses" v-else-if=" client &&client.courses===null">
            <small
    ...
    <br/>
       ...
       <td  data-label="Courses" v-else-if="  client &&client.other_courses===null">
        <small v-for="(client3, index1) in  client.courses"
        :key="index1%2===1"  class="text-gray-500 dark:text-gray-300 capitalize"> 
        <small v-if="client3.course">
          <span >{{client3.course.name}}</span> -- <span class="text-emerald-400 text-md"> {{client3.branch.name }}</span>
  ...
  <!-- <div v-if=" client" >
        <td  data-label="Branch" v-if=" client &&client.courses===null">
            <small
        v-for="(client3, index1) in  client.other_courses"
        :key="index1%2===1"  class="text-gray-500 dark:text-gray-300 capitalize"> 
     ...
     <td data-label="Status"> 
          <small v-if="client.records?.length > 0" class="text-gray-400 dark:text-gray-300 font-semibold">
            <span v-if="client.records.slice(-1)[0].status">{{client.records.slice(-1)[0].status.name}}-{{client.records.slice(-1)[0].status.substatus.name}}
              <!-- <span v-if="client.records.slice(-1)[0].status.substatus"> -  </span> -->
