...
const email =ref("")
const number =ref("")
const recs = computed(() => props.joined);
...
const perPage = 50;
const itemsCount =computed(()=>followup.joined_total)
const itemsPaginated = computed(() =>followup.joined);
...
const pageChange = async (page) => {
    ...
    // } else {
        await followup.getjoined(page,recs.value);
    // }
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getjoined(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getjoined(currentPage.value,recs.value );
    }
...
const joined=computed(()=>{
   return followup.joined  
})

onBeforeMount(async (page) => {
     pageLoading.value = true
  await followup.getjoined(currentPage.value-1,recs.value) 
     pageLoading.value = false 
})
