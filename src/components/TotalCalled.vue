...
const recs = computed(() => props.totalcalled);
const currentPage = ref(0);

const hasError = ref(false)
const perPage = 50;
const itemsCount =computed(() =>followup.totalcalled_total);
const itemsPaginated = computed(() =>followup.totalcalled);
...
const pageChange = async (page) => {
    ...
    // } else {
        await followup.gettotalcalled(page,recs.value);
    // }
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.gettotalcalled(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.gettotalcalled(currentPage.value,recs.value );
    }
...
const called=computed(()=>{
   return followup.totalcalled  
})

onBeforeMount(async () => {
     pageLoading.value = true
  await followup.gettotalcalled(currentPage.value-1,recs.value)
     pageLoading.value = false  
})
</script>
...
<template>
 <div v-if="pageLoading" wire:loading
  ...
  >
    ...
    <card-component
      class="shadow-lg w-full h-screen md:w-5/5 z-50 mt-[46px] overflow-y-scroll"
       title="Total Called data" 
    ...
    <div>
      ...
      >
        ...
        <field label="Search Name" class="ml-[5px]">  
         ...
         <input type=text v-model="email"   class="border border-success bg-gray-800 w-full h-[35px] rounded pl-1" placeholder="Enter name here" required />
         </field>

          <field label="Search Number" class="ml-[5px]">  
         <input type=text v-model="number"   class="border border-success bg-gray-800 w-full h-[35px] rounded pl-1" placeholder="Enter name here" required />
         </field>
...
<!-- {{client.education==null}} --> 
      ...
      <span v-for="(client1, index1) in client.contact" :key="index1%2===1" >
        ...
        <span v-for="(client2, index1) in client.email" :key="index1%2===1">
         <small class="text-gray-500 dark:text-gray-300 ">
         <a class=""  :href="'mailto:' +   client2.email">  
