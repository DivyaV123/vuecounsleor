...
import { mdiClose, mdiPlusBox, mdiMinusBox, mdiEmailPlusOutline,mdiEmail,mdiMicrophone,mdiBookEducation,mdiFilter } from '@mdi/js'
...
const email =ref("")
const number =ref("")
const recs = computed(() => props.otherjoined);
...
const perPage = 50;
const itemsCount =computed(()=>followup.other_joined_total)
const itemsPaginated = computed(() =>followup.other_joined);
...
const pageChange = async (page) => {
    ...
    // } else {
        await followup.getotherjoined(page,recs.value);
    // }
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getotherjoined(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getotherjoined(currentPage.value,recs.value );
    }
...
onBeforeMount(async (page) => {
     pageLoading.value = true
  await followup.getotherjoined(currentPage.value-1,recs.value) 
     pageLoading.value = false 
})
...
<!-- {{client.courses}} -->
       <td  data-label="Courses" v-if=" client.courses&&client.other_courses">
        ...
        <small v-if="client3.course">
          ...
          <td  data-label="Courses" v-else-if="client.courses===null">
            <small
