...
import { mdiPlusBox, mdiMicrophone,mdiTextBoxEdit, mdiAccountPlus } from "@mdi/js";
import ModalBox from "@/components/ModalBox.vue";
import ModalNumberRecord from "@/components/ModalNumberRecord.vue";
// import ModalAddStatus from "@/components/ModalAddStatus.vue";
...
const itemsPaginated = computed(() => {
    if(counselor.updateStatus && counselor.updateStatus.items){
        return counselor.updateStatus.items
    } 
}
);
...
const addEnquiry = (id,name) => {
  isModalActive.value = true;
  propValues.id = id;
  propValues.name=name
};

const confirmAddEnquiry = async () => {
  counselor.submitAddStatus({
    id: propValues.id,
    status: {
      id: 1,
      name: "enquiry",
      substatus: { id: 1, name: "enquiry" }
    },
  });
  router.push({ path: "/add-enquiry", query: { rid: propValues.id } });
};
...
<template>
  <modal-box v-model="isModalActive" :title="'Enquiry' +' - '+ propValues.name" buttonLabel="Add Enquiry" has-cancel
    @confirm="confirmAddEnquiry">
    <p>Are you sure you want to <b>add-enquiry</b>?</p>
  </modal-box>
  ...
  <!-- <modal-add-status v-if="isModalStatusActive" v-model="isModalStatusActive"
    :title="'Update status of ' + propValues.name" :id="propValues.id" @confirm="confirmRefresh" has-cancel>
  </modal-add-status> -->
  <modal-change-status v-if="isModalStatusActive" v-model="isModalStatusActive"
    :title="'Update status of ' + propValues.name" :id="propValues.id"
  ...
  <table class="mt-5">
    ...
    <tbody>
      <tr v-for="(client, index) in itemsPaginated" :key="client.id"
        ...
        <td data-label="Audio" class="flex justify-center">
          <small
            v-if="client.record" 
        ...
        <td data-label="Record" class="text-center">
          <!-- <jb-button :label="client.record_count" color="info" :icon="mdiMicrophone" small
            @click="openRecordModal(client.name)" /> -->
        ...
        <td data-label="Status" class="text-center">
          <small>
            <jb-button label="Add Status" class="font-bold" color="cyan" :icon="mdiPlusBox" small
        ...
        <td data-label="Actions" class="text-center">
          ...
          <small v-if="client.EnquiryID==null">
            <jb-button label="Add Enquiry"  class="font-semibold" color="amber" :icon="mdiAccountPlus" small @click="addEnquiry(client.id,client.name)" />
          </small>
