...
const email =ref("")
const number =ref("")
const recs = computed(() => props.notjoined);
...
const perPage = 50;
const itemsCount =computed(()=>followup.not_joined_total)
const itemsPaginated = computed(() =>followup.not_joined);
...
const pageChange = async (page) => {
    ...
    await followup.getnotjoined(page,recs.value);
...
const searchParams = async() => {
 ...
 params = new URLSearchParams(searchdata).toString();
   ...
   currentPage.value=0
    await  followup.getnotjoined(currentPage.value,recs.value,params);
}
}
 ...
 number.value="";
    currentPage.value=0;
    await  followup.getnotjoined(currentPage.value,recs.value );
    }
onBeforeMount(async (page) => {
     pageLoading.value = true
  await followup.getnotjoined(currentPage.value-1,recs.value) 
     pageLoading.value = false 
})
